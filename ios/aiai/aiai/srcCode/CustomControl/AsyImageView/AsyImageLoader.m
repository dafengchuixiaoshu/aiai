...
- (void)initializeImageFile
{
    ...
    _imageFile = [libraryDir stringByAppendingPathComponent:@"ImageFile"];
...
- (void)loadImageWithPath:(NSString *)path
{
    if ([path hasPrefix:@"http://"]) {
        ...
        NSString *filePath = [_imageFile stringByAppendingPathComponent:fileEncodeName];
        ...
        [self loadImageFromLocalWithPath:path];
    }
}
...
{
    dispatch_queue_t loadimageQueue = dispatch_queue_create("loadimage", NULL);
...
- (void)finishGettingWithImage:(UIImage *)image
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(AsyImageLoader:finishedLoadingImage:Style:)]) {
            [self.delegate AsyImageLoader:self finishedLoadingImage:image Style:self.displayStyle];
        }
    });
}
