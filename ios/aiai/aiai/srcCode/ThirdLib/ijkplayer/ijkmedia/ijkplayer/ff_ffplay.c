...
int autorotate=0;
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, FFPlayer* ffp , AVFrame *frame)
{
	const char *vfilters=ffp->vfilters;
//	ALOGE("configure_video_filters: %s \n",vfilters);
...
//    ALOGE("avfilter_graph_create_filter: %d \n",ret);
    if (ret < 0)
        goto fail;
...
static int configure_audio_filters(VideoState *is,FFPlayer* ffp , int force_output_format)
{
	const char *afilters=ffp->afilters;
    ...
    do {
        ...
        if (got_frame) {
                ...
                if (reconfigure) {
                    ...
                    if ((ret = configure_audio_filters(is, ffp, 1)) < 0)
                        goto the_end;
                }
...
static int ffplay_video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            if ((ret = configure_video_filters(graph, is, ffp, frame)) < 0) {
//                SDL_Event event;
//                event.type = FF_QUIT_EVENT;
//                event.user.data1 = is;
//                SDL_PushEvent(&event);
//                ALOGE("configure_video_filters return： %d \n",ret);
                goto the_end;
            }
...
static int video_thread(void *arg)
{
    ...
    int       ret = 0;
//    ALOGE("video_thread start from ffplay_sdl \n");
    if (ffp->node_vdec) {
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
#if CONFIG_AVFILTER
        {
            ...
            if ((ret = configure_audio_filters(is, ffp, 0)) < 0)
                goto fail;
...
void ffp_create_required_filter(FFPlayer *ffp,FilterDataStructure* data)
{
#if CONFIG_AVFILTER
    if(!ffp || !data) return;
    ...
    //filter_type
    char *effect = NULL;
    if(data->filter_type >= FilterType_Raw)
    {
        switch (data->filter_type) {
            case FilterType_BW:
            {
                effect = "lutyuv='u=128:v=128'";
            }
                break;
            case FilterType_Fuzzy:
            {
                effect = "boxblur=1:10:4:10";
            }
                break;
            case FilterType_Magic:
            {
                effect = "hue='H=2*PI*t:s=sin(2*PI*t)+1'";
            }
                break;
            case FilterType_Nagative:
            {
                effect = "curves=negative";
            }
                break;
            case FilterType_Vintage:
            {
    ...
    if(data->first_movie_path){
        char *afilter = "amovie=%s,volume=volume=%d,adelay=%.1f|%.1f[w];[in]volume=volume=%d[ain];[ain][w]amix";
        size_t afilterLength = 512;
        ...
        ffp->afilters = (char*)malloc(afilterLength);
        memset(ffp->afilters, 0, afilterLength);
        if(data->start_time<0.0000001 && data->start_time > -0.0000001){
            data->start_time = 0.001;
            data->end_time += data->start_time;
        }
        sprintf(ffp->afilters, afilter,data->first_movie_path,data->is_open_volume,data->start_time*1000,data->start_time*1000,data->is_open_volume);
    }
    //vafilter
    if (effect == NULL && data->first_movie_path==NULL && data->second_movie_path==NULL) return;

    char* vfilter = NULL;
    if (effect == NULL){
        if (data->first_movie_path){
            if (data->second_movie_path){
                vfilter = "movie=%s,setsar=0[v0],movie=%s,setsar=0[v1];[in]fps=fps=%d[vin];[v0][v1]alphamerge,fps=fps=%d,setpts=PTS+%.1f/TB[alpha];[vin][alpha]overlay=0:0:enable=between(t\\,%.1f\\,%.1f)";
            }else{
                vfilter = "movie=%s,setsar=sar=0,fps=fps=%d,setpts=PTS+%.1f/TB[w];[in]fps=fps=%d[vin];[vin][w]blend=all_expr='if(lte(B,128),A,B)':enable=between(t\\,%.1f\\,%.1f)";
            }
        }else{
            if (data->second_movie_path){
                vfilter = "movie=%s,setsar=sar=0,fps=fps=%d,setpts=PTS+%.1f/TB[w];[in]fps=fps=%d[vin];[vin][w]blend=all_expr='if(lte(B,128),A,B)':enable=between(t\\,%.1f\\,%.1f)";
        ...
        if (data->first_movie_path){
            if (data->second_movie_path){
                vfilter = "movie=%s,setsar=0[v0],movie=%s,setsar=0[v1];[in]fps=fps=%d[vin];[v0][v1]alphamerge,fps=fps=%d,setpts=PTS+%.1f/TB[alpha];[vin][alpha]overlay=0:0:enable=between(t\\,%.1f\\,%.1f)%s%s";
            }else{
                vfilter = "movie=%s,setsar=sar=0,fps=fps=%d,setpts=PTS+%.1f/TB[w];[in]fps=fps=%d[vin];[vin][w]blend=all_expr='if(lte(B,128),A,B)':enable=between(t\\,%.1f\\,%.1f)%s%s";
            }
        }else{
            if (data->second_movie_path){
                vfilter = "movie=%s,setsar=sar=0,fps=fps=%d,setpts=PTS+%.1f/TB[w];[in]fps=fps=%d[vin];[vin][w]blend=all_expr='if(lte(B,128),A,B)':enable=between(t\\,%.1f\\,%.1f)%s%s";
    ...
    size_t vfilterLength = 1024;
    ffp->vfilters = (char*)malloc(vfilterLength);
    memset(ffp->vfilters, 0, vfilterLength);
    if(effect == NULL)
    {
        if (data->first_movie_path){
            if (data->second_movie_path){
                sprintf(ffp->vfilters, vfilter, data->first_movie_path,data->second_movie_path,data->fps,data->fps,data->start_time,data->start_time,data->end_time);
            }else{
                sprintf(ffp->vfilters, vfilter, data->first_movie_path,data->fps,data->start_time,data->fps,data->start_time,data->end_time);
            }
        }else{
            if (data->second_movie_path){
                sprintf(ffp->vfilters, vfilter, data->second_movie_path,data->fps,data->start_time,data->fps,data->start_time,data->end_time);
            }else{
                return;
            }
        }
        ...
        if (data->first_movie_path){
            if (data->second_movie_path){
                sprintf(ffp->vfilters, vfilter, data->first_movie_path,data->second_movie_path,data->fps,data->fps,data->start_time,data->start_time,data->end_time,"[v1];[v1]",effect);
            }else{
                sprintf(ffp->vfilters, vfilter, data->first_movie_path,data->fps,data->start_time,data->fps,data->start_time,data->end_time,"[v1];[v1]",effect);
            }
        }else{
            if (data->second_movie_path){
                sprintf(ffp->vfilters, vfilter, data->second_movie_path,data->fps,data->start_time,data->fps,data->start_time,data->end_time,"[v1];[v1]",effect);
            }else{
                sprintf(ffp->vfilters, vfilter,effect);
            }
        }
    }
#endif
