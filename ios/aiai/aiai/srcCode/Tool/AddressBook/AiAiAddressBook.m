...
static AiAiAddressBook *instance;
@implementation AiAiAddressBook
-(id)init
{
    if (self=[super init]) {
...
// 单列模式
+ (AiAiAddressBook*)shareControl{
    @synchronized(self) {
        if(!instance) {
            instance = [[AiAiAddressBook alloc] init];
        }
    }
    return instance;
}
...
#pragma mark 获取通讯录内容
-(void)getPersonInfoComplete:(GetContactDic)complete{
...
//        NSLog(@"%@",dic);
        ...
        NSString *strFirLetter = [NSString stringWithFormat:@"%c",[ChineseToPinyin sortSectionTitle:[str substringToIndex:0]]];
