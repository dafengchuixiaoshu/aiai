...
- (id)initWithFrame:(CGRect)frame andVedioLength:(CGFloat)length
{
    ...
    if (self) {
        ...
        self.leftIndex = 0;
        self.maxLines = length * ONE_SECOND_SCORE_LINES+1;
...
-(void)getLinesPositions{
    ...
    for (int i=0; i<count; i++) {
        ...
        NSNumber* str = [NSNumber numberWithFloat:0.2*(i+self.leftIndex)];
...
-(void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx{
    if (layer == self.layer) {
        ...
        CGContextSetLineCap(ctx, kCGLineCapRound);
        CGContextSetStrokeColorWithColor(ctx,Every_YellowColor.CGColor);
        CGContextStrokePath(ctx);
    }
}
