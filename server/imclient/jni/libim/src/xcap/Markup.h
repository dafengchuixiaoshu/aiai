...
public:
	...
	///< 从头开始找，lkx add
	bool FindElemFromStart ( LPCTSTR szName=NULL );
...
//yqs添加
	bool AddElem( LPCTSTR szName, double dbValue, int nFlags=0 ) { return x_AddElem(szName,dbValue,nFlags); };
	bool InsertElem( LPCTSTR szName, double dbValue, int nFlags=0 ) { return x_AddElem(szName,dbValue,nFlags|MNF_INSERT); };
	bool AddChildElem( LPCTSTR szName, double dbValue, int nFlags=0 ) { return x_AddElem(szName,dbValue,nFlags|MNF_CHILD); };
	bool InsertChildElem( LPCTSTR szName, double dbValue, int nFlags=0 ) { return x_AddElem(szName,dbValue,nFlags|MNF_INSERT|MNF_CHILD); };

//yqs添加
	bool AddElem( LPCTSTR szName, float fValue, int nFlags=0 ) { return x_AddElem(szName,fValue,nFlags); };
	bool InsertElem( LPCTSTR szName, float fValue, int nFlags=0 ) { return x_AddElem(szName,fValue,nFlags|MNF_INSERT); };
	bool AddChildElem( LPCTSTR szName, float fValue, int nFlags=0 ) { return x_AddElem(szName,fValue,nFlags|MNF_CHILD); };
	bool InsertChildElem( LPCTSTR szName, float fValue, int nFlags=0 ) { return x_AddElem(szName,fValue,nFlags|MNF_INSERT|MNF_CHILD); };
...
//yqs添加
	bool AddAttrib( LPCTSTR szAttrib, double dbValue ) { return x_SetAttrib(m_iPos,szAttrib,dbValue); };
	bool AddChildAttrib( LPCTSTR szAttrib, double dbValue ) { return x_SetAttrib(m_iPosChild,szAttrib,dbValue); };
//yqs添加
	bool AddAttrib( LPCTSTR szAttrib, float fValue ) { return x_SetAttrib(m_iPos,szAttrib,fValue); };
	bool AddChildAttrib( LPCTSTR szAttrib, float fValue ) { return x_SetAttrib(m_iPosChild,szAttrib,fValue); };
...
//yqs添加
	bool SetAttrib( LPCTSTR szAttrib, double dbValue ) { return x_SetAttrib(m_iPos,szAttrib,dbValue); };
	bool SetChildAttrib( LPCTSTR szAttrib, double dbValue ) { return x_SetAttrib(m_iPosChild,szAttrib,dbValue); };

//yqs添加
	bool SetAttrib( LPCTSTR szAttrib, float fValue ) { return x_SetAttrib(m_iPos,szAttrib,fValue); };
	bool SetChildAttrib( LPCTSTR szAttrib, float fValue ) { return x_SetAttrib(m_iPosChild,szAttrib,fValue); };
...
//yqs添加
	bool SetData( float fValue ) { return x_SetData(m_iPos,fValue); };
	bool SetChildData( float fValue) { return x_SetData(m_iPosChild,fValue); };
	...
	// yqs增加
	void GetDataEx(int& nData){nData = atoi(GetData());};
	void GetDataEx(double& dbData){dbData = atof(GetData());};
	void GetDataEx(float& fData){fData = (float)atof(GetData());};

	void GetAttribEx(LPCTSTR szAttrib, int& nData){nData = atoi(GetAttrib(szAttrib));};
	void GetAttribEx(LPCTSTR szAttrib, double& dbData){dbData = atof(GetAttrib(szAttrib));};
	void GetAttribEx(LPCTSTR szAttrib, float& fData){fData = (float)atof(GetAttrib(szAttrib));};
	
	void GetChildAttribEx(LPCTSTR szAttrib, int& nData){nData = atoi(GetChildAttrib(szAttrib));};
	void GetChildAttribEx(LPCTSTR szAttrib, double& dbData){dbData = atof(GetChildAttrib(szAttrib));};
	void GetChildAttribEx(LPCTSTR szAttrib, float& fData){fData = (float)atof(GetChildAttrib(szAttrib));};


	CString MemToStr(char* pBuf, int nLen);	
	int	StrToMem(char* pBuf, int nBufLen, CString strBlock);//内存块字符化,返回数据长度(字节)nBufLen内存区大小
...
//yqs添加
	bool x_AddElem( LPCTSTR szName, double dbValue, int nFlags );
	bool x_AddElem( LPCTSTR szName, float fValue, int nFlags );
...
//yqs添加
	bool x_SetAttrib( int iPos, LPCTSTR szAttrib, double dbValue);
	bool x_SetAttrib( int iPos, LPCTSTR szAttrib, float fValue );
...
//yqs添加
	...
	bool x_SetData( int iPos, float fValue );
