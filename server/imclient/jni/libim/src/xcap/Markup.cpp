...
bool CMarkup::ReadTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
	...
	if ( pstrError )
		(*pstrError).Format(_T("%s%d bytes to %d wide chars"),(LPCSTR ) szDescBOM,nFileByteLen,nWideLen);
	...
	if ( pstrError )
		(*pstrError).Format( _T("%s%d bytes"), (LPCSTR ) szDescBOM, nFileByteLen );
#endif
	fclose( fp );
	if ( pnFlags )
		*pnFlags = nFlags;
...
bool CMarkup::WriteTextFile( LPCTSTR szFileName, CString& strDoc, CString* pstrError, int* pnFlags )
{
	...
	bool bSuccess = true;
	FILE* fp = fopen( szFileName, _T("wb") );
	if ( ! fp )
	{
		if ( pstrError )
			*pstrError = strerror(errno);
...
bool CMarkup::FindElemFromStart( LPCTSTR szName )
{
	// Change current position only if found
	//
	if ( m_aPos.GetSize() )
			...
			if ( nMatchDepth == 0 )
			{
				...
				if ( m_strError.IsEmpty() )
				{
					m_strError.Format( _T("No start tag for end tag '%s' at offset %d"),
						(LPCSTR ) x_GetToken(token), aNodes.Top().nStart );
				}
			}
			else
			{
				...
				if ( m_strError.IsEmpty() )
				{
					if ( nTypeFound == 0 )
						m_strError.Format( _T("End tag '%s' at offset %d does not match start tag '%s' at offset %d"),
							(LPCSTR ) x_GetToken(token), token.nL-1, (LPCSTR ) aNodes.Top().strMeta, pElem->nStart );
					else
						m_strError.Format( _T("Element '%s' at offset %d not ended"),
							(LPCSTR ) aNodes.Top().strMeta, aNodes.Top().nStart );
				}
			}
			if ( nTypeFound == -2 )
				break;
...
int CMarkup::x_ParseNode( CMarkup::TokenPos& token, CMarkup::NodePos& node )
{
	...
	#define FINDNODEBAD(e) { szFindEnd=_T(">"); nEndLen=1; node.strMeta.Format(_T("Incorrect %s at offset %d"),(LPCSTR ) e,nR); nNodeType=-1; }
...
bool CMarkup::x_SetAttrib( int iPos, LPCTSTR szAttrib, double dbValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%f"), dbValue );
	return x_SetAttrib( iPos, szAttrib, szVal );
}
bool CMarkup::x_SetAttrib( int iPos, LPCTSTR szAttrib, float fValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
...
bool CMarkup::x_SetData( int iPos, float fValue )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%f"), fValue );
...
bool CMarkup::x_AddElem( LPCTSTR szName, double dbValue, int nFlags )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%f"), dbValue );
...
bool CMarkup::x_AddElem( LPCTSTR szName, float fValue, int nFlags )
{
	// Convert integer to string
	_TCHAR szVal[25];
	_stprintf( szVal, _T("%f"), fValue );
...
CString CMarkup::MemToStr(char* pBuf, int nLen)
{
	...
	{
		strTmp.Format("%02X", pBuf[i]&0x0ff);
		strRet += strTmp;
	}
	return strRet;
}
int	CMarkup::StrToMem(char* pBuf, int nBufLen, CString strBlock)
{
	if(pBuf == NULL)
	{
		return 0;
	...
	for(int i = 0 ; i < nLen; i++)
	{
		sscanf(strBlock.Mid(i*2,2),"%X",&nData);
		pBuf[i] = nData & 0xff;
	}
	return nLen;
}
