...
//			#define ASSERT(f) assert((f))
			#define ASSERT(f)
//			#define ASSERT(f) if (!(f)) { printf ("%d:%s:%s", __LINE__, __FILE__, "ASSERT FALSE"); }
		#endif
		#ifndef VERIFY
			#ifdef _DEBUG
...
// as an LPCTSTR.
	...
	typedef const wchar_t*			PCOLESTR;
...
#define wcscmp wcscmp2
#define wcsncmp wcsncmp2
#define wcsstr wcsstr2
#define wcscpy wcscpy2
#define wcsncpy wcsncpy2

int wcslen2 (const wchar_t* s);
int wcscmp2 (const wchar_t* s1, const wchar_t* s2);
int wcsncmp2 (const wchar_t* s1, const wchar_t* s2, int n);
wchar_t* wcsstr2 (const wchar_t* s1, const wchar_t* s2);
void wcscpy2 (wchar_t* dst, const wchar_t* src);
void wcsncpy2 (wchar_t* dst, const wchar_t* src, int n);

inline BSTR SysAllocString (const WCHAR* bstr) {
	///< ���ָ��Ϊ�գ��ͷ���һ�����ַ�����
	if (bstr == NULL) {
		BSTR bstrRet = (BSTR ) malloc (sizeof (WCHAR) * 1);
		bstrRet [0] = 0;
		return bstrRet;
	}

	int len = wcslen (bstr);
	BSTR bstrRet = (BSTR ) malloc (sizeof (WCHAR) * (len + 1));
	wcscpy (bstrRet, bstr);
	return bstrRet;
}

inline BSTR SysAllocStringLen (const WCHAR* bstr, int len) {
	BSTR bstrRet = (BSTR ) malloc (sizeof (WCHAR) * (len + 1));
	if (bstr != NULL) {
		wcscpy (bstrRet, bstr);
	}
...
inline int SysStringByteLen (const WCHAR* str) {
	int len = wcslen (str);
	return len * 2;
}

inline int SysStringLen (const WCHAR* str) {
	int len = wcslen (str);
	return len;
}
...
inline BSTR SysAllocStringByteLen (const wchar_t* bstr, int len) {
	BSTR bstrRet = new WCHAR [len + 1];
	if (bstr != NULL) {
		wcsncpy (bstrRet, bstr, len);
	}
	else {
		bstrRet [0] = 0;
	}
...
inline char* ConvertBSTRToString (BSTR pSrc) {
	int len = wcslen (pSrc);
	char* ret = NULL;
	unicode_to_utf8 ( (BSTR ) pSrc, len * sizeof (wchar_t), (uint8_t** ) &ret);
	return ret;
}

inline BSTR ConvertStringToBSTR (const char* src) {
	BSTR bstr = NULL;
	int out_size;
...
#else
	char* dst = ConvertBSTRToString ( (BSTR ) pW);
	sDst = dst;
	free (dst);
#endif
}
...
public:
	...
	///< lkx add
	CStdStr(wchar_t* wstr, int nSize) {
		char* utf8 = NULL;
		unicode_to_utf8 (wstr, nSize * sizeof (wchar_t), (uint8_t** ) &utf8);
		ssasn(*this, SSREF(utf8));
		free (utf8);
	}
...
//		}
	...
	{
		if ( static_cast<int>(std::basic_string<CT>::size()) < nMinLen )
    ...
    void Format(const CT* szFmt, const A1& v1, const A2& v2, const A3& v3,
                ...
                const A8& v8, const A9& v9, const A10& v10, const A11& v11,
                const A12& v12, const A13& v13, const A14& v14, const A15& v15,
                const A16& v16, const A17& v17, const A18& v18)
    {
        Fmt(szFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(),
            FmtArg<A3>(v3)(), FmtArg<A4>(v4)(), FmtArg<A5>(v5)(),
            FmtArg<A6>(v6)(), FmtArg<A7>(v7)(), FmtArg<A8>(v8)(),
	...
	// -------------------------------------------------------------------------
		...
		BSTR AllocSysString() const
		{
			BSTR bstrTmp = ConvertStringToBSTR (*this);
			return bstrTmp;
		}
	#endif
